...
lambda = alfa; // chute inicial do autovalor usado para realizar as iterações
...
while k <= M,
    x1 = Gaussian_Elimination_4(A - lambda * eye(n, n), x0); // resolve o sistema (A - alfa * I) * x1 = x0 na variável x1 a fim de iterar inversamente
    x1 = x1 / norm(x1, 2); // normalização de x1
    lambda = x1' * A * x1; // calcula o quociente de Rayleigh
    if x1' * x0 < 0 then // verifica se x1^T * x0 é negativo // alteração feita para que x1 mantenha o sentido de x0
